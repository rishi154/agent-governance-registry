...
function cardHTML(item) {
  ...
  <div onclick="openDetail('${item.id}')"
    ...
    ${caps}
    <div class="mt-3 flex flex-wrap gap-1">${badges}${authBadge}${isBlocked ? '<span class="text-xs px-1.5 py-0.5 rounded font-medium badge-blocked">📝 Human Review Pending</span>' : isLiveUnreviewed ? '<span class="text-xs px-1.5 py-0.5 rounded font-medium bg-amber-50 text-amber-700">⚠ Live — Unreviewed</span>' : '<span class="text-xs px-1.5 py-0.5 rounded font-medium badge-approved">✓ Allowed</span>'}</div>
  </div>`;
}
...
async function openDetail(itemId) {
  ...
  const enforcementBadge = isBlocked
    ? `<div class="mt-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-800 font-semibold">
        📝 HUMAN REVIEW PENDING — This ${isAgent ? 'agent' : 'tool'} is awaiting admin approval. Status: ${item.status}.
        ${item.status === 'rejected' ? 'It was rejected by an admin.' : item.ai_review ? 'AI review complete — awaiting human decision.' : 'AI review in progress.'}
      </div>`
    : isLiveUnreviewed
    ? `<div class="mt-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-800 font-semibold">
